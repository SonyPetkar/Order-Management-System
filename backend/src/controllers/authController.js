...
export const login = async (req, res) => {
  try {
    ...
    if (!isMatch) {
      console.warn(`Login failed - invalid password for user: ${normalizedEmail}`);
      return res.status(401).json({
        success: false,
        message: 'Invalid credentials',
      });
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    // For now, return token (NOT FOR PRODUCTION)
    console.log(`Password reset token for ${email}: ${resetToken}`);

    res.status(200).json({
      success: true,
      message: 'Password reset token sent to email. Use this token to reset password.',
